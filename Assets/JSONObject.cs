...
#if UNITY_EDITOR 
//|| UNITY_ANDROID || UNITY_IOS || UNITY_STANDALONE || UNITY_WEBPLAYER || UNITY_WII || UNITY_PS3 || UNITY_XBOX360 || UNITY_FLASH
...
namespace Boomlagoon.JSON
{
	...
	public class JSONValue {
		...
		public JSONValue(decimal number) {
        ...
        public int Int { get { return (int)Number; } }
        public long Long { get { return (long)Number; } }
        public ulong Ulong { get { return (ulong)Number; } }
        public float Float { get { return (float)Number; } }

		public static implicit operator JSONValue(string str) {
			return new JSONValue(str);
		}

		public static implicit operator JSONValue(decimal number) {
			return new JSONValue(number);
		}

        public static implicit operator JSONValue(double number) {
            return new JSONValue((decimal)number);
        }

        public static implicit operator JSONValue(ulong number)
        {
            return new JSONValue(number);
        }
        ...
        public static implicit operator JSONValue(int number)
        {
            return new JSONValue(number);
        }

        public static implicit operator JSONValue(uint number)
        {
            return new JSONValue(number);
        }
...
#if UNITY_EDITOR
        public string ToBeautifyString(int tabCount, bool isArray)
        {
            switch (Type)
            {
                case JSONValueType.Object:
	...
	public class JSONArray : IEnumerable<JSONValue> {
        ...
        public string ToBeautifyString(int tabCount)
        {
            ...
            {
                stringBuilder.Append(values[i].ToBeautifyString(tabCount + 1, true));
        ...
        public bool[] ToArrayBool()
        {
            ...
            {
                if(values[i].Type == JSONValueType.Boolean)
                {
                    list.Add(values[i].Boolean);
                }
            }
            return list.ToArray();
        }
        ...
        public int[] ToArrayInt()
        {
            ...
            for (int i = 0; i < values.Count; i++)
            {
                if (values[i].Type == JSONValueType.Number)
                {
                    list.Add((int)values[i].Number);
                }
            }
            return list.ToArray();
        }
        ...
        public float[] ToArrayFloat()
        {
            ...
            for (int i = 0; i < values.Count; i++)
            {
                if (values[i].Type == JSONValueType.Number)
                {
                    list.Add((float)values[i].Number);
                }
            }
            return list.ToArray();
        }
        ...
        public string[] ToArrayString()
        {
            ...
            for (int i = 0; i < values.Count; i++)
            {
                if (values[i].Type == JSONValueType.String)
                {
                    list.Add(values[i].Str);
                }
            }
            return list.ToArray();
        }
    ...
    public class JSONObject : IEnumerable<KeyValuePair<string, JSONValue>> {
		...
		private static decimal ParseNumber(string str, ref int startPosition) {
			...
			decimal result;
			if (
				!decimal.TryParse(str.Substring(startPosition, endPosition - startPosition), System.Globalization.NumberStyles.Float,
        ...
        public string ToBeautifyString(int tabCount = 0, bool isArray = false)
        {
#if UNITY_EDITOR
            var stringBuilder = new StringBuilder();
